...
엑셀에서 각각 워드파일과 텍스트파일을 선택한다.
그런다음 텍스트파일의 내용 중에 조건에 맞는 부분을 워드파일에 삽입한다. 
텍스트 파일에서 다음과 같은 형태의 문장이 있다
...
기존 워드 파일을 열어서 작업한다 
텍스트 파일에서 "-"로 시작하는 첫 번째 라인만 처리하면 된다
워드 파일에 텍스트를 삽입할 때 위치만 지정하면 된다.
위치는 페이지나눔 앞에 삽입하는 것으로 한다.
