...
Sub ImportQuestionFromWord()
    ...
    For patternIndex = 0 To a1Count - 1  ' すべてのA1パターンを処理
        ...
        Debug.Print "パターン" & (patternIndex + 1) & "の処理を開始します。targetRow = " & targetRow
    ...
    ' 마지막 패턴의 마지막 "Answer:" 찾기
    For potentialEnd = startLine To UBound(allLines)
        If Left(Trim(allLines(potentialEnd)), 7) = "Answer:" Then
            answerCount = answerCount + 1
            If answerCount = 3 Then  ' (11), (12), (13)의 Answer까지 찾음
                endLine = potentialEnd + 1  ' Answer 다음 줄까지 포함
                Debug.Print "最後のパターンの終了位置を発見: " & potentialEnd & " (Answer:" & answerCount & "個目)"
                Exit For
            End If
        End If
    Next potentialEnd
    
    Debug.Print "最後のパターンの範囲: " & startLine & " から " & endLine
Else
...
End If
        ...
        ' 最後のパターンの特別な処理のためのフラグ
        Dim isLastPattern As Boolean
        isLastPattern = (patternIndex = a1Count - 1)
        ...
        For i = 0 To UBound(lines)
            ...
            ' 最後のパターンの場合、デバッグ情報を追加
            If isLastPattern Then
                Debug.Print "最後のパターン処理中: 行 " & i & " / " & UBound(lines) & " 内容: " & Left(currentLine, 10)
            End If
            ...
            If Left(currentLine, 2) = "A1" Then
                ...
                ' デバッグ情報を出力
                Debug.Print "問題番号: " & currentLine & ", 行インデックス: " & i & ", 配列サイズ: " & UBound(lines) & ", 現在のtargetRow: " & targetRow
            ...
            ElseIf Left(currentLine, 4) = "(13)" Then
                ...
                If i + 5 <= UBound(lines) And Left(Trim(lines(i + 5)), 7) = "Answer:" Then
                    ...
                    Debug.Print "(13) 解答を処理: " & answer
                End If
                
                ' パターン処理完了フラグを設定
                patternProcessed = True
                Debug.Print "パターン処理完了フラグを設定: TRUE"
            End If
...
NextLine:
        ...
        Else
            ' 마지막 패턴이고 완전히 처리되지 않았다면 강제로 행 증가
            If patternIndex = a1Count - 1 And Not patternProcessed Then
                ' 마지막 패턴에 대한 특별 처리 - A1은 있지만 (13)까지 없는 경우
                Debug.Print "最後のパターンは(13)までないため、特別処理を適用します"
                
                ' 마지막 패턴이 정상적으로 처리되었다고 가정
                patternProcessed = True
        ...
        ' 最後のパターンで特別な処理が必要な場合の追加チェック
        If patternIndex = a1Count - 1 And Not patternProcessed Then
            Debug.Print "最後のパターンが完全に処理されませんでした。特別な手続きを実行します。"
            
            ' このパターンのA1行の内容はすでに処理済みのはず
            ' さらなる検査とデバッグのために、各セルの値を確認
            Debug.Print "最後のパターンのセル内容:"
            Debug.Print "A列: " & targetSheet.Cells(targetRow, COL_AAAA).Value
            Debug.Print "B列: " & targetSheet.Cells(targetRow, COL_BBBB).Value
            Debug.Print "C列: " & targetSheet.Cells(targetRow, COL_EEEE).Value
            
            ' 実際にパターンの処理が行われたことを確認するために、未設定の場合は値を設定
            If IsEmpty(targetSheet.Cells(targetRow, COL_EEEE).Value) Then
                Debug.Print "長文が見つかりませんでした。適切な情報を設定します。"
                For j = 0 To UBound(lines)
                    If j > startLineOfLongText And j < UBound(lines) Then
                        longText = longText & lines(j) & vbCrLf
                    End If
                Next j
                targetSheet.Cells(targetRow, COL_EEEE).Value = longText
    ...
    ' 필요없는 행 삭제 (startRow + a1Count 이후의 행)
    Dim deleteStartRow As Long
    deleteStartRow = 3 + a1Count  ' startRow(3) + a1Count
    
    ' deleteStartRow부터 50행을 삭제
    Dim deleteEndRow As Long
    deleteEndRow = deleteStartRow + 50  ' 충분히 많은 행을 삭제
    
    Debug.Print deleteStartRow & "行目から" & deleteEndRow & "行目までの不要な行を削除します"
    targetSheet.Rows(deleteStartRow & ":" & deleteEndRow).Delete
    
    ' 넘버링 추가 및 테두리 설정
    Dim i As Long
    For i = 3 To 3 + a1Count - 1
        ' A열에 넘버링 추가
        targetSheet.Cells(i, 1).Value = i - 2  ' 1부터 시작하는 넘버링
        
        ' 테두리 설정
        With targetSheet.Range(targetSheet.Cells(i, 1), targetSheet.Cells(i, COL_13CCCC))
