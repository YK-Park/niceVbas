'word target tool 250220-02.bas
Option Explicit
...
Public Sub ProcessWords()
    ...
    If resultCount >= startRow Then
       ...
       '// ステップ4の修正部分: C,D列の単語を比較して同じ語幹の単語から最短のものをE列に保存
        For i = startRow To resultCount
            ...
            If wsC.Cells(i, "E").Value <> "" Then
                ...
                Dim shortestStem As String
                shortestStem = wsC.Cells(i, "D").Value
                ...
                For j = startRow To resultCount
                    If j <> i And wsC.Cells(j, "E").Value <> "" Then
                        If HasSameStem(wsC.Cells(i, "E").Value, wsC.Cells(j, "E").Value) Then
                            ...
                            currentWord = wsC.Cells(j, "E").Value
                            Dim currentStem As String
                            currentStem = wsC.Cells(j, "D").Value
                            
                            If Len(currentWord) < Len(shortestWord) Then
                                shortestWord = currentWord
                                shortestStem = currentStem
                                shortestIndex = j
                            End If
                        End If
                    End If
