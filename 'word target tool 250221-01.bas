...
Public Sub ProcessWords()
    ...
    For i = startRow To lastRowA
        ...
        If isValid Then
            For j = startRow To resultCount
                If wsB.Cells(j, "F").Value <> "" Then
                    If HasSameStem(currentWord, wsB.Cells(j, "F").Value, similarityThreshold) Then
                        isValid = False
                        Exit For
                    End If
                End If
        ...
        If isValid Then
            ...
            wsB.Cells(resultCount, "C").Value = wsA.Cells(i, "A").Value  '// 級番号
            wsB.Cells(resultCount, "D").Value = wsA.Cells(i, "B").Value  '// ユニーク番号
            wsB.Cells(resultCount, "E").Value = wsA.Cells(i, "C").Value  '// 級
            wsB.Cells(resultCount, "F").Value = wsA.Cells(i, "D").Value  '// ターゲット単語
            wsB.Cells(resultCount, "G").Value = wsA.Cells(i, "E").Value  '// 品詞
            wsB.Cells(resultCount, "H").Value = wsA.Cells(i, "F").Value  '// 出題区分
        End If
    Next i
    ...
    If resultCount >= startRow Then
        For i = startRow To resultCount
            If wsB.Cells(i, "F").Value <> "" Then
                Dim shortestWord As String
                shortestWord = wsB.Cells(i, "F").Value
                Dim shortestIndex As Long
                shortestIndex = i
                
                For j = i + 1 To resultCount
                    If wsB.Cells(j, "F").Value <> "" Then
                        If HasSameStem(shortestWord, wsB.Cells(j, "F").Value, similarityThreshold) Then
                            If Len(wsB.Cells(j, "F").Value) < Len(shortestWord) Then
                                shortestWord = wsB.Cells(j, "F").Value
                                shortestIndex = j
                            End If
                        End If
                    End If
                ...
                If shortestIndex <> i Then
                    wsB.Range("C" & i & ":H" & i).ClearContents
                Else
                    For j = i + 1 To resultCount
                        If wsB.Cells(j, "F").Value <> "" Then
                            If HasSameStem(shortestWord, wsB.Cells(j, "F").Value, similarityThreshold) Then
                                wsB.Range("C" & j & ":H" & j).ClearContents
                            End If
                        End If
                    Next j
                End If
        ...
        For i = startRow To resultCount
            If wsB.Cells(i, "F").Value <> "" Then
                If i <> newRow Then
                    wsB.Range("C" & i & ":H" & i).Copy
                    wsB.Range("C" & newRow & ":H" & newRow).PasteSpecial xlPasteValues
                    wsB.Range("C" & i & ":H" & i).ClearContents
                End If
                newRow = newRow + 1
            End If
        ...
        '// 不要な結果行をクリア
        If newRow <= resultCount Then
            wsB.Range("C" & newRow & ":H" & resultCount).ClearContents
        End If
    End If
