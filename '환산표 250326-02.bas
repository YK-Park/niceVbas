...
Private Function IsValidXlsxFilename(filename As String) As Boolean
    ...
    ' 条件1: 特定の文字列が含まれているか（例: "データ"）
    Dim condition1 As Boolean
    condition1 = InStr(1, baseName, "データ") > 0
    
    ' 条件2: 特定の文字列が含まれているか（例: "処理"）
    Dim condition2 As Boolean
    condition2 = InStr(1, baseName, "処理") > 0
    
    ' 条件3: 4桁の数字-1桁の数字のパターンが含まれているか（例: "1234-5"）
    ...
    With regEx
        ...
        .Global = True
        .IgnoreCase = True
        condition3 = .Test(baseName)
    End With
    
    ' 少なくとも一つの条件を満たす必要がある
    IsValidXlsxFilename = condition1 Or condition2 Or condition3
    
    ' すべての条件を満たす必要がある場合は以下のようにする
    ' IsValidXlsxFilename = condition1 And condition2 And condition3
End Function
...
Private Sub ProcessFiles(csvFilePath As String, xlsxFilePath As String, resultFilePath As String, _
    ...
    For i = 2 To lastRow  ' ヘッダーをスキップ
        ...
        ' B列の変換 - 参照シートを使用して変換
        Dim rawBValue As String
        rawBValue = CStr(xlsWs.Cells(i, 2).Value)
        
        ' A列の値に基づいてB列の値を変換
        bValue = ConvertValueIfMatch(aValue, rawBValue, 2, 50)  ' 行範囲は2～50としています（必要に応じて調整）
        
        ' F列の変換 - A列の値に基づいて変換
        Dim rawFValue As String
        rawFValue = CStr(xlsWs.Cells(i, 6).Value)  ' F列の値
        
        ' ヘッダー情報から変換を試みる
        If headerInfo.Exists(rawFValue) Then
            fValue = headerInfo(rawFValue)
        Else
            ' A列の値に基づいてF列の値を変換
            fValue = ConvertValueIfMatch(aValue, rawFValue, 2, 50)
            
            ' 変換できなかった場合はデフォルト値
            If fValue = rawFValue Then
                fValue = "0000XXX"  ' デフォルト値
            End If
        End If
        
        ' G列の値 - A列の値に基づいて変換
        Dim rawGValue As String
        rawGValue = CStr(xlsWs.Cells(i, 7).Value)  ' G列の値
        
        ' A列の値に基づいてG列の値を変換
        gValue = ConvertValueIfMatch(aValue, rawGValue, 2, 50)
