...
' ファイル名から抽出した条件値を格納するグローバル変数
Private g_condition1Value As String
Private g_condition2Value As String
Private g_fourDigits As String
Private g_oneDigit As String
...
Private Function ExtractPatternFromFilename(filename As String) As String
    ' すでに抽出されている場合はそれを使用
    If g_fourDigits <> "" And g_oneDigit <> "" Then
...
' ファイル名から条件値を抽出する関数 - グローバル変数を使用
Private Function ExtractConditionsFromFilename(filename As String) As Boolean
    Dim fso As Object
    Set fso = CreateObject("Scripting.FileSystemObject")
    Dim baseName As String
    baseName = fso.GetFileName(filename)
    ...
    If InStr(1, baseName, "データ") > 0 Then
        g_condition1Value = "データ"
    End If
    
    ' 条件2: "処理"が含まれているか
    If InStr(1, baseName, "処理") > 0 Then
        g_condition2Value = "処理"
    End If
    ...
    ExtractConditionsFromFilename = (g_condition1Value <> "" Or g_condition2Value <> "" Or (g_fourDigits <> "" And g_oneDigit <> ""))
End Function

' XLSXファイル名が条件を満たしているか確認する関数
Private Function IsValidXlsxFilename(filename As String) As Boolean
    ' 抽出関数を使用して条件を確認 - グローバル変数を使用
    IsValidXlsxFilename = ExtractConditionsFromFilename(filename)
End Function
...
Private Sub ProcessFiles(csvFilePath As String, xlsxFilePath As String, resultFilePath As String, _
    ...
    hasConditions = ExtractConditionsFromFilename(xlsxFilePath)
